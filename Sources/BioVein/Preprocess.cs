...
namespace BioVein
{
    public class Preprocess
    {
        ...
        private static ORB _features;
        
        public static (Mat output, Mat roi) FrameTick(Mat frame)
        {
            _features = ORB.Create(scoreType: ORBScoreType.Harris, nFeatures: 1500, edgeThreshold: 0);
            ...
            if (roi.Width != 0 && roi.Height != 0)
            {
                ...
                using (var roiSource = new Mat(context.Green, roi))
                {
                    ...
                    if (roiSource.Height > 0 && roiSource.Width > 0 && roiSource.Height == DefaultRoiSize && roiSource.Width == DefaultRoiSize)
                    {
                        ...
                        CvXImgProc.Thinning(deNoised, Program.DoesOrbEnabled ? deNoised : outRoi);

                        if (Program.DoesOrbEnabled)
                        {
                            var (points, _) =
                                ORBMatcher.CalculateKeyPoints(_features, deNoised); // get features keypoints
                            Cv2.DrawKeypoints(deNoised, points, outRoi, Scalar.Red);
                        }
                    ...
                    // using (var rr = context.Source.Clone())
                    // {
                    //     Cv2.Rectangle(rr, roi, Scalar.White);
                    //     Cv2.ImWrite("ROI.jpg", rr);
                    // }
                }
            }
            context.Dispose();
